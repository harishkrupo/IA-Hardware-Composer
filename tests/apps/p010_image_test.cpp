...
static int
create_dmabuf_buffer(struct buffer *buffer,
...
static bool image_open(struct image *image,
		       const char *filename)
{
...
static struct image *
image_create(const char *filename)
{
...
void copy_buffer_to_handle(struct gbm_handle *handle, buffer *buffer) {
	...
	handle->import_data.fd_modifier_data.offsets[1] = buffer->stride * buffer->height;
...
main(int argc, char *argv[])
{
	...
	layer.SetSourceCrop(hwcomposer::HwcRect<float>(0, 0,
						       1920, 1080));
